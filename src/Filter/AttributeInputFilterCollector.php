...
use Modularity\Attribute\ArrayInput;
use Modularity\Attribute\Input;
use Modularity\Attribute\ObjectInput;
use Modularity\Filter\ObjectInputFilter;
use ReflectionClass;
...
class AttributeInputFilterCollector
{
    ...
    public function __construct(
        ...
        $this->cacheFile = APP_ROOT . '/data/cache/input_filters.cache.php';
    }
    ...
    private function createFromArray(array $config, array $data): InputFilter
    {
        ...
        foreach ($config as $item) {
            if ($item['type'] === 'input') {
                $this->injectAdapter($item['validators']);
                $inputFilter->add([
                    'name'       => $item['name'],
    ...
    private function injectAdapter(?array &$validators): void
    {
        ...
        foreach ($validators as &$validator) {
            if (
                isset($validator['options']['table']) &&
                $this->adapter instanceof AdapterInterface &&
                ! isset($validator['options']['adapter'])
            ) {
                $validator['options']['adapter'] = $this->adapter;
            }
        }
    }
}
