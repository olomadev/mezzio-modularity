...
/**
 * Runs the "Modularity-post-install-cmd" and "remove" scripts defined in the composer.json module.
 *
 * "extra": {
 *    "Modularity-post-install-cmd": [
...
class ModuleComposerScriptRunner
{
    ...
    public function run(string $moduleFullPath)
    {
        ...
        if ($this->command == "install" && ! empty($moduleComposer['extra']['Modularity-post-install-cmd'])) {
            $postInstallCmds = $moduleComposer['extra']['Modularity-post-install-cmd'] ?? [];
            foreach ($postInstallCmds as $cmd) {
                $this->output->writeln("<info>Running module post-install-cmd: $cmd.</info>");
                passthru($cmd . " --ansi");
            }
        }
        if ($this->command == "remove" && ! empty($moduleComposer['extra']['Modularity-post-remove-cmd'])) {
            $postRemoveCmds = $moduleComposer['extra']['Modularity-post-remove-cmd'] ?? [];
